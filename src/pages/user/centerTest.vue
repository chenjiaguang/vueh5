        ...
        <header ref="topHeader" class="top-header" :style="{backgroundImage: 'url(' + user.cover + ')'}">
          <div class="top-header-left">
            <div class="name-and-gender">
              ...
              <i v-if="user.sex && (user.sex.toString() === '1' || user.sex.toString() === '2') || true" class="user-gender iconfont" :class="{'icon-gender1': user.sex && user.sex.toString() === '1' || true, 'icon-gender2': user.sex && user.sex.toString() === '2'}"></i>
            </div>
            <div class="constellation-and-location" v-if="user.constellation || user.location || true">
              <div class="user-constellation">{{user.constellation}}要你管</div>
              <div class="user-location">{{user.location}}地址就随便写吧</div>
            </div>
            <div class="vip-user" v-if="user.is_vip_user || true"><i class="iconfont icon-vip vip-icon"></i>{{user.vip_intro}}认证什么的最麻烦了</div>
...
export default {
  ...
  methods: {
    ...
    refreshData () {
      ...
      let _initialData = JSON.parse(JSON.stringify(initialData))
      let _obj = Object.assign({}, _initialData, {selectedIdx, selectedLabel, swiperOption, swiper, swiperSlide})
