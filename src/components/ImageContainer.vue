  ...
  <div style="width: 100%;overflow: hidden;">
    <template v-for="(item, idx) in images">
      <transition :key="item.sign || item.id || idx" :appear="appearAnimation" appear-class="before-appear">
        <div :key="item.sign || item.id || idx" @click="previewImage(idx)" :style="{marginTop: idx < 3 ? 0 : '1.055%'}" class="item-container fl" :class="{disabled: !(item.localUrl || item.url), one: images.length === 1 && !isUpload, 'two-and-more': images.length > 1 || isUpload, left: idx % 3 === 0, horizontal: Number(item.width) >= Number(item.height), vertical: Number(item.width) < Number(item.height)}">
